...
(function(root, factory) {
  ...
  return function() {
    ...
    function directionSouth() {
      ...
      // oveflow right only
      if (bbox.s.x + node.offsetWidth / 2 > screenRect.width && bbox.s.y + 250 < window.innerHeight) {
        diff = (bbox.s.x + node.offsetWidth / 2) - screenRect.width
        return {
          top: bbox.s.y,
          left: bbox.s.x - node.offsetWidth / 2 - diff - 10
        }
      }

      // overflow left only
      if (bbox.s.x - node.offsetWidth / 2 < 0 && bbox.s.y + 250 < window.innerHeight) {
        diff = node.offsetWidth / 2 - bbox.s.x
        return {
          top: bbox.s.y,
          left: bbox.s.x - node.offsetWidth / 2 + diff + 10
        }
